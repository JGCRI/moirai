...
## LDS Framework
This section focuses on the meta-structure of the LDS framework with the aim of providing a background for using the system. Complementarily, the basic processing flow is depicted in [Figures 1 <sup>3</sup>](https://stash.pnnl.gov/projects/JGCRI/repos/lds/browse/docs/usr_gd_fig_1.png) and [2 <sup>4</sup>](https://stash.pnnl.gov/projects/JGCRI/repos/lds/browse/docs/usr_gd_fig_2.png). The LDS framework consists primarily of C code and is contained within the …/lds2 directory, with the exception of three input data sets that need to be downloaded from the internet by the user. The downloaded input data sets can be stored anywhere because their location is set in the LDS input file (…/lds2/lds_input.txt). The user will also need to download and install the C NetCDF library to read on of these downloaded data sets. The rest of the input data are in …/lds2/indata (this directory is set in the LDS input data file), including the two files that specify the GLUs (which are also set in the LDS input file). The LDS outputs go into a directory within  …/lds2/outputs that is specified in the LDS input file. A runtime log file (lds_log.txt) is also written to the …/lds2/outputs directory, the name of which is also set in the LDS input file.

Five R scripts are also included in the framework. The …/lds2/indata/convert_wfgrids2binary.r script converts the water footprint files (downloaded by the user) from ARC binary grids to simple binary raster images for input to the LDS (because the linked GDAL library in the C code would not recognize the original files). The four R scripts in …/lds2/outputs generate various diagnostic outputs, and need some of the LDS diagnostic output files in order to run. The LDS outputs used by the GCAM data system are copied by the LDS into the more accessible …/aglu-data/LDS and …/aglu-data/mappings directories (the names of these files and their destination directories are also set in the LDS input file).

The LDS is a command line tool that takes the name of the LDS input file as the sole argument. The LDS can be run within Xcode on a Mac, or compiled anywhere (hopefully) using the …/lds2/makefile (which is for Linux and has been tested on the Mac terminal). The easier way to use the LDS is by opening the …/lds2/lds.xcodeproj file in Xcode, then compiling and running it directly within Xcode, mainly because most of the conditions for running it are already established (location of the NetCDF library may have to be changed in the build settings, and also in lds.h). The current default setting for the location of the executable is …/lds2/build/Products/Debug. The makefile, however, puts the executable in …/lds2/bin, and the objects in …/lds2/obj, but it is easier to call lds from …/lds2 because of the input file default path entries. The user must first create these two directories and also change the location of the NetCDF library in both the makefile and in lds.h. Regardless of how the code is run, the user must also correctly specify the input and output directories (and any other input data files that they may want to substitute) in the LDS file before running the code.
...
## Inputs
This section focuses on the inputs listed in the LDS input file. Of the many inputs to the LDS, those listed in the LDS input file are the most important as they include the primary source data in addition to the GLU definition that determines how to aggregate the source data. The extent and resolution of the default raster input data determine the working resolution of the LDS, which is defined in the LDS header file (…/lds2/lds.h; global extent, 5 arcmin resolution). When substituting input data the user must ensure that the new data are read in and resampled to the working grid. An alternative grid can also be defined in the LDS header file. Each input has its own read function that can be rewritten to accommodate input data set substitution.

### LDS input file (.../lds2/lds_input.txt)
...
* inpath: path to directory containing all input files except for the SAGE 175 crops, MIRCA2000, and water footprint data (…/lds2/indata)
* outpath: path to directory where all output files will be written (this directory must already exist) (…/lds2/outputs/<user_specified>)
