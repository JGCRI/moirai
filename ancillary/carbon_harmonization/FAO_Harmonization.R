...
num_neighbor_list <- 40 #can set this to a list (ie "c(1,10,20,40)") if want to run using multiple different search boxes
...
for(a in num_neighbor){
  ...
  for (i in states_of_carbon){
    ...
    for (j in c(IGBP_classes[1:nrow(IGBP_classes),])){
      ...
      if(topsoil_only){
        tmp_raster <- as.data.frame(rasterToPoints(raster(paste0("Soil_carbon_rasters/",j,"_topsoil_",i,".bil"))))
