...
create_carbon_plots_soil_detail<-function(fig_dir  = "carbon_plots/",
                                   plot_lim = 1000, 
                                   path_to_carbon_data = "../example_outputs/basins235/Ref_veg_carbon_Mg_per_ha.csv",
                                   path_to_carbon_rasters = "../indata/",
                                   path_to_mapping = "./glu_mapping_basin235.csv",
                                   ...
                                   path_to_iso_mapping = "../indata/FAO_iso_VMAP0_ctry.csv",
                                   path_to_land_data = "../example_outputs/basins235/Land_type_area_ha.csv",
                                   path_to_land_mapping = "../example_outputs/basins235/MOIRAI_land_types.csv",
                                   glu_name = "Missouri_River_Basin",
                                   carbon_year = 2010,
                                   carbon_input_src_name = "soil_grids",
                                   print_raster_progress_log =FALSE)
{
  ...
  Carbon_data <- read.csv(path_to_carbon_data,skip=5,stringsAsFactors = FALSE)
  Land_data <- read.csv(path_to_land_data,skip=5,stringsAsFactors = FALSE)
  land_type_mapping <- read.csv(path_to_land_mapping,skip=4,stringsAsFactors = FALSE) %>% dplyr::select(Category, LT_SAGE) %>% rename(land_type = Category)
  iso_data <- read.csv(path_to_iso_mapping,stringsAsFactors = FALSE) %>% 
    rename(ctry_id=fao_code,iso=iso3_abbr)
  ...
  #Get the soil carbon data and clean the same.
  Carbon_data %>% 
    left_join(land_type_mapping, by =c("land_type")) %>% 
    filter(c_type=="soil_c (0-100 cms)") %>%
    rename(wavg=weighted_average) %>% 
    distinct() %>% 
    inner_join(iso_data,by=c("iso")) %>% 
  ...
  #Aggregate carbon data across land types.  
  soil_carbon_data %>% 
    dplyr::select(glu_code,land_type,wavg,median_value,min_value,max_value,q1_value,q3_value,land_area, land_type) %>% 
    group_by(glu_code, land_type) %>% 
    mutate(wavg=sum(as.numeric(wavg)*as.numeric(land_area))/sum(as.numeric(land_area)),
           median_value=median(median_value),
           min_value=min(min_value),
           max_value=max(max_value),
           ...
           q3_value=quantile(q3_value,0.75)) %>% 
    ungroup() %>% 
    dplyr::select(glu_code,wavg,median_value,min_value,max_value,q1_value,q3_value,land_type) %>% 
    distinct()->soil_carbon_agg
  if(print_raster_progress_log == TRUE){
    write.csv(soil_carbon_agg, "soil_carbon_agg.csv")
  ...
  g<-ggplot(data=data_for_plot,aes(x=value)) +
    ...
    geom_point(data=data_for_plot,aes(x=median_value,y=plot_lim,shape="median_value",color=land_type),show.legend = TRUE,size=3,alpha=0.9)+
    geom_point(data=data_for_plot,aes(x=min_value,y=plot_lim,shape="min_value",color=land_type),show.legend = TRUE,size=3,alpha=0.9)+
    geom_point(data=data_for_plot,aes(x=max_value,y=plot_lim,shape="max_value",color=land_type),show.legend = TRUE,size=3,alpha=0.9)+
    geom_point(data=data_for_plot,aes(x=q1_value,y=plot_lim,shape="q1_value",color=land_type),show.legend = TRUE,size=3,alpha=0.9)+
    geom_point(data=data_for_plot,aes(x=q3_value,y=plot_lim,shape="q3_value",color=land_type),show.legend = TRUE,size=3,alpha=0.9)+
    geom_point(data=data_for_plot,aes(x=wavg,y=plot_lim,shape="weighted_average",color=land_type),show.legend = TRUE,size=3,alpha=0.9)+
    ggtitle(paste0("Soil carbon distribution for ",toString(glu_name), " in MgC/ha (0-100 cm depth), distribution from ", toString(carbon_input_src_name)))+
    labs(subtitle = "Shapes represent initialization values from moirai outputs for each category")
