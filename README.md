...
## LDS Framework
This section focuses on the meta-structure of the LDS framework with the aim of providing a background for using the system. Complementarily, the basic processing flow is depicted in [Figures 1 <sup>3</sup>](https://stash.pnnl.gov/projects/JGCRI/repos/lds/browse/docs/usr_gd_fig_1.png) and [2 <sup>4</sup>](https://stash.pnnl.gov/projects/JGCRI/repos/lds/browse/docs/usr_gd_fig_2.png). The LDS framework consists primarily of C code and is contained within the …/lds project directory (in src and include directories), along with all input data. Three publically available data sets are also included, and can be moved or downloaded separately because their location is set in the LDS input file (e.g., …/lds/input_files/lds_input_basins235.txt). The user will need to download and install the C NetCDF library to read one of these data sets. The input data are in …/lds/indata (this directory is set in the LDS input data file), including the two files that specify the GLUs (which are also set in the LDS input file). The LDS outputs (main and diagnostic) go into a directory within the …/lds directory that is specified in the LDS input file (e.g., …/lds/outputs/basins235). A runtime log file (e.g., lds_log_basins235.txt), the name of which is also set in the LDS input file, is also written to the specified output directory. The ten main output files used by the GCAM data system are also copied by the LDS into directories specified in the LDS input file (one directory for data and one for mappings).

Five R scripts are also included in the framework. The four R scripts in …/lds/diagnostics generate various diagnostic outputs, and need some of the LDS diagnostic output files in order to run. The …/lds/indata/WaterFootprint/convert_wfgrids2binary.r script was used to convert the water footprint files from ARC binary grids to simple binary raster images for input to the LDS (because the linked GDAL library in the C code would not recognize the original files).

The LDS is a command line tool that takes the name of the LDS input file as the sole argument. The LDS can be run within Xcode on a Mac, or compiled anywhere (hopefully) using the …/lds/makefile (which is for Linux and has been tested on the Mac terminal). The easier way to use the LDS is by opening the …/lds/lds.xcodeproj file in Xcode, then compiling and running it directly within Xcode, mainly because most of the conditions for running it are already established (the location of the NetCDF library may have to be changed in the build settings, and also in lds.h; the project directory is the working directory, and the name of the input file needs to be set in the scheme). The current default setting for the location of the executable is …/lds/Build/Products/Debug. The makefile, however, puts the executable in …/lds/bin, and the objects in …/lds/obj, but this executable needs to be called from from …/lds because the example input file path entries are based on this project directory as the working directory (these can be changed, as needed). The user must first change the location of the NetCDF library in both the makefile and in lds.h. Regardless of how the code is run, the user must also correctly specify the input and output directories (and any other input data files that they may want to substitute) in the LDS file before running the code.

The LDS production outputs are independent of the GCAM regions. However, there are diagnostic outputs that do aggregate data to the GCAM land use regions based on either 14 or 32 GCAM regions. These aggregated diagnostic outputs are used by some of the diagnostic R scripts. The default is 32 regions in the example input files, but the user can specify which GCAM region set to use in the LDS input file, by pointing to two files originally copied directly from …/gcam-data-system/_common/mappings.

## Required downloads and installs
Only the NetCDF library has to be downloaded and installed by the user, as the three data sets below are now included in the repository through the LFS sytem.
...
The LDS generates the following three files and copies them to a user-specified destination directory (e.g., …/lds/outputs/basins235/aglu-data; these files replace the GTAP data previously stored in …/aglu-data/level0):
...
The LDS also generates these files (some of which were previously stored in either the deprecated …/aglu-data/GIS directory or …/aglu-data/level0, may have been previously produced from the old “GIS” code), and copies them to user-specified destination directory (e.g., …/lds/outputs/basins235/aglu-data):
...
These two mapping files are generated and copied to a user-specified mappings destination directory (e.g., …/lds/outputs/basins235/aglu-data):
...
## Inputs
This section focuses on the inputs listed in the LDS input file. Of the many inputs to the LDS, those listed in the LDS input file are the most important as they include the primary source data in addition to the GLU definition that determines how to aggregate the source data. The extent and resolution of the default raster input data determine the working resolution of the LDS, which is defined in the LDS header file (…/lds/include/lds.h; global extent, 5 arcmin resolution). When substituting input data the user must ensure that the new data are read in and resampled to the working grid. An alternative grid can also be defined in the LDS header file. Each input has its own read function that can be rewritten to accommodate input data set substitution.

### LDS input file (e.g., .../lds/input_files/lds_input_basins235.txt)
...
* outpath: path to directory where all output files will be written (e.g., …/lds/outputs/<user_specified>)
...
* ldsdestpath: path to directory where the eight LDS output data files will be copied to (e.g., …/aglu-data/LDS)
* mapdestpath: path to directory where the two LDS output mapping files will be copied to (e.g., …/aglu-data/mappings)
...
* lds_log_basins235.txt: LDS runtime log file (this should be case-specific)
...
## Diagnostics
There are four R scripts in …/lds/outputs that generate a variety of figures. Make sure that …/lds/outputs is the R working directory before running the scripts. Each script writes to a user-specified directory within the outputs directory. Set this diagnostic output directory within each script. Each script has a detailed description at the beginning, and comments identifying the relevant directories, files, and flags that the user can change to customize the outputs. To diagnose country-level harvested area and production outputs, use “plot_lds_crop_ctry.r.” To diagnose land use region level harvested area and production outputs, use “plot_lds_crop_raez.r.” Note that the difference statistics and the KS tests in "plot_lds_crop_raez.r" are invalid when the geographic land units are not identical among the different data sets. To diagnose the land type area output, use “plot_lds_land_type_area.r.” To diagnose the land rent output (at different aggregation levels), use “plot_lds_landrent_aez.r”. Note that the difference statistics and KS tests in "plot_lds_landrent_aez.r" are invalid when the geographic land units are not identical among the different data sets.
