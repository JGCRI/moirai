...
Alternatively, `moirai` can be compiled using the `makefile`, with similar modifications to specify the location of your NetCDF library. Open `…/moirai/makefile` with any text editor and change the text following "-L" in `LDFLAGS_GENERIC = -lm -L/usr/local/lib -lnetcdf` to reflect the location of your NetCDF library. Also change the path in `NCHDRDIR = /usr/local/include` to reflect the location of your NetCDF header file.

Then navigate to the `…/moirai` directory on the command line and type `make`.
