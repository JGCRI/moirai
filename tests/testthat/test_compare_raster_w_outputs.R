...
test_that("Compare raster outputs with LDS outputs at basin level", {
  ...
  LDS_data %>%left_join(ISO %>% dplyr:::select(iso,GCAM_region_ID),by="iso") ->LDS_data
  ...
  Land_Data_Actual<-(as.data.frame(rasterToPoints(Land_Data_Binary))) 
  Basin_Data_Raster<-(as.data.frame(rasterToPoints(Basin_raster)))
...
test_that("Compare raster outputs with LDS outputs at country level", {
  ...
  #Convert raster files to data frames
  Land_Data_Actual<-(as.data.frame(rasterToPoints(Land_Data_Binary))) 
  Country_Data_raster<-(as.data.frame(rasterToPoints(Country_raster)))
