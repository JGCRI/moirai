/**********
 write_raster_float.c
 
 write a float raster image
 start at upper left corner and write row by row (this is how the data are stored)
 no header
 
 arguments:
 float out_array[]:		array to write to file
...
int write_raster_float(float out_array[], int out_length, char *out_name, args_struct in_args) {
	...
	{
		fprintf(fplog,"Failed to open file %s: write_raster_float()\n", fname);
		return ERROR_FILE;
	}
	...
	{
		fprintf(fplog, "Error writing file %s: write_raster_float(); records written=%i != out_length=%i\n",
